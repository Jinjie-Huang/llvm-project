...
// RUN: split-file %s %t
//
// RUN: %clang_cc1  -Wno-header-shadowing -std=c++20 %t/foo.cppm -I%t -emit-module-interface -o %t/foo.pcm
// RUN: %clang_cc1  -Wno-header-shadowing -fprebuilt-module-path=%t -std=c++20 %t/use.cpp -I%t/. -fsyntax-only -verify

// RUN: %clang_cc1  -Wno-header-shadowing -std=c++20 %t/foo.cppm -I%t -emit-reduced-module-interface -o %t/foo.pcm
// RUN: %clang_cc1  -Wno-header-shadowing -fprebuilt-module-path=%t -std=c++20 %t/use.cpp -I%t/. -fsyntax-only -verify
