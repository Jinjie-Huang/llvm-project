...
extern "C" {
int64_t __bolt_bray_dynamic_offset;
uint64_t __bolt_bray_resolved_entry_hook;
uint64_t __bolt_bray_resolved_exit_hook;
}
...
static void *resolveHook(const char *HookName) {
  uint64_t *Dynamic = reinterpret_cast<uint64_t *>(
      reinterpret_cast<uint64_t>(&__bolt_bray_dynamic_offset) +
      __bolt_bray_dynamic_offset);
...
extern "C" void *__bolt_bray_resolve_entry() {
  return resolveAndCache(&__bolt_bray_resolved_entry_hook, EntryHookName);
}

extern "C" void *__bolt_bray_resolve_exit() {
  return resolveAndCache(&__bolt_bray_resolved_exit_hook, ExitHookName);
}

} // namespace

extern "C" __attribute((naked)) void __bolt_bray_entry_dispatch() {
  // clang-format off
#if defined(__x86_64__)
  __asm__ __volatile__("mov __bolt_bray_resolved_entry_hook(%%rip), %%r11\n"
                       "test %%r11, %%r11\n"
                       "jz 1f\n"
                       "cmp $2, %%r11\n"
                       "jbe 3f\n"
                       ...
                       "1:\n"
                       SAVE_ALL
                       "call __bolt_bray_resolve_entry\n"
                       "test %%rax, %%rax\n"
                       "jz 2f\n"
                       "mov %%rax, 40(%%rsp)\n"
                       RESTORE_ALL
  ...
  __asm__ __volatile__("adrp x16, __bolt_bray_resolved_entry_hook\n"
                       "ldr x16, [x16, #:lo12:__bolt_bray_resolved_entry_hook]\n"
                       "cbz x16, 1f\n"
                       "cmp x16, #2\n"
                       "b.ls 3f\n"
                       "br x16\n"
...
extern "C" __attribute((naked)) void __bolt_bray_exit_dispatch() {
  // clang-format off
#if defined(__x86_64__)
  __asm__ __volatile__("mov __bolt_bray_resolved_exit_hook(%%rip), %%r11\n"
                       "test %%r11, %%r11\n"
                       "jz 1f\n"
                       "cmp $2, %%r11\n"
                       "jbe 3f\n"
                       ...
                       :::);
#elif defined(__aarch64__) || defined(__arm64__)
  __asm__ __volatile__("adrp x16, __bolt_bray_resolved_exit_hook\n"
                       "ldr x16, [x16, #:lo12:__bolt_bray_resolved_exit_hook]\n"
                       "cbz x16, 1f\n"
                       "cmp x16, #2\n"
                       "b.ls 3f\n"
                       "br x16\n"
