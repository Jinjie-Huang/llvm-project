//===- bolt/Passes/BRayInstrumentation.cpp --------------------------------===//
//
...
#include "bolt/Passes/BRayInstrumentation.h"
...
Error BRayInstrumentation::runOnFunctions(BinaryContext &BC) {
  ...
  const bool ExternalHook = BRayRuntimeLibrary::needsRuntime(BC);
  ...
  if (opts::instrumentFunctionEntry())
    EntryCallTarget =
        findHook(BRayRuntimeLibrary::EntryHookName);
  if (opts::instrumentFunctionExit())
    ExitCallTarget = findHook(BRayRuntimeLibrary::ExitHookName);
  ...
  if (opts::instrumentFunctionEntry() && !EntryCallTarget)
    EntryCallTarget = createLocalDispatch(
        "__bolt_bray_local_entry_dispatch",
        BRayRuntimeLibrary::EntryDispatchOffsetName);
  if (opts::instrumentFunctionExit() && !ExitCallTarget)
    ExitCallTarget = createLocalDispatch(
        "__bolt_bray_local_exit_dispatch",
        BRayRuntimeLibrary::ExitDispatchOffsetName);
  if ((opts::instrumentFunctionEntry() && !EntryCallTarget) ||
