...
namespace opts {
cl::opt<std::string> RuntimeBRayLib(
    "runtime-bray-lib",
    cl::desc("specify path of the BRay function probe resolver library"),
    cl::init("libbolt_rt_bray.a"), cl::cat(BoltInstrCategory));
} // namespace opts

bool BRayRuntimeLibrary::needsRuntime(const BinaryContext &BC) {
...
void BRayRuntimeLibrary::adjustCommandLineOptions(
    const BinaryContext &BC) const {
  assert(opts::isBRayInstrumentation() &&
         "BRay runtime requires function instrumentation");
...
void BRayRuntimeLibrary::emitBinary(BinaryContext &BC, MCStreamer &Streamer) {
  assert(opts::isBRayInstrumentation() &&
         "BRay runtime requires function instrumentation");
  MCSection *Section = BC.Ctx->getELFSection(
      ".bolt.bray", ELF::SHT_PROGBITS,
...
void BRayRuntimeLibrary::link(BinaryContext &BC, StringRef ToolPath,
                              BOLTLinker &Linker,
                              BOLTLinker::SectionsMapper MapSections) {
  assert(opts::isBRayInstrumentation() &&
         "BRay runtime requires function instrumentation");
  std::string LibPath = getLibPath(ToolPath, opts::RuntimeBRayLib);
