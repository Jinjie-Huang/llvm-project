# BRay Function Entry/Exit Instrumentation
...
```text
probe site
  -> __bolt_bray_local_{entry,exit}_dispatch
  -> __bolt_bray_{entry,exit}_dispatch
  -> __bolt_probe_{enter,exit}
```

The local dispatcher loads a signed relative offset from the corresponding
`__bolt_bray_{entry,exit}_dispatch_offset` slot, adds it to the slot address,
